...
def camera_to_lidar_box(boxes, V2C=None, R0=None, P2=None):
    ...
    for box in boxes:
        x, y, z = box
        (x, y, z)= camera_to_lidar(x, y, z, V2C=V2C, R0=R0, P2=P2)
...
def lidar_to_camera_box(boxes, V2C=None, R0=None, P2=None):
    ...
    for box in boxes:
        x, y, z = box
        (x, y, z) = lidar_to_camera(x, y, z, V2C=V2C, R0=R0, P2=P2)
...
def center_to_corner_box3d(boxes_center, coordinate='lidar'):
    # (N, 7) -> (N, 8, 3)
    N = boxes_center.shape[0]
    ret = np.zeros((N, 3, 3), dtype=np.float32)
    ...
    for i in range(N):
        ...
        cornerPosInVelo = np.tile(translation, (4, 1)).T
